...
public class Diversos
{
    ...
    #region Iniciais
    private static void ExecutaSqlANTIGO<TENTIDADE, TMODEL>(DbContext context)
    ...
    public static TEntity ParseSqlToEntity<TEntity>(string sql, DbContext dbContext, Guid? createdBy = null) where TEntity : class, new()
    {
        ...
        // Extrair os valores com um Regex robusto
        var valuesString = match.Groups[2].Value;
        var values = ExtractValues(valuesString);



        if (columnNames.Length != values.Count)
    ...
    // Método auxiliar para extrair valores do SQL
    private static List<string> ExtractValues(string valuesString)
    {
        var values = new List<string>();
        var regex = new Regex(@"'([^']*)'|([^,()]+)", RegexOptions.Compiled);
        var matches = regex.Matches(valuesString);

        // Lista para manter o controle de quantos valores temos
        string? currentValue = null;

        foreach (Match match in matches)
        {
            if (match.Groups[1].Success) // Valor entre aspas simples
            {
                if (currentValue != null)
                {
                    currentValue += "," + match.Groups[1].Value; // Concatenar múltiplos valores entre aspas
                }
                else
                {
                    currentValue = match.Groups[1].Value; // Primeiro valor entre aspas
                }
            }
            else if (match.Groups[2].Success) // Valor sem aspas simples (número, booleano, etc.)
            {
                if (currentValue != null)
                {
                    values.Add(currentValue); // Adicionar o valor atual
                    currentValue = null; // Limpar o valor atual
                }
                values.Add(match.Groups[2].Value.Trim()); // Adicionar o valor sem aspas
            }
        }

        if (currentValue != null) // Adicionar o último valor entre aspas
        {
